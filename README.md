...
> [!TIP]
> **Customization Notice — Illustrative Examples (For Example)**:
> All subfolder names and note titles listed in the table below are **illustrative examples (`for example`)** provided to demonstrate vault organization and naming standards. You should customize the subfolders and notes according to your own research fields, career goals, programming languages, and personal interests.

| Domain | Scope & Knowledge Focus | Subfolder Structure (For Example) | Representative Notes (For Example) |
| :--- | :--- | :--- | :--- |
| **`academic/`** | Research papers, scientific theories, mathematics, study modules. | *(e.g., `machine-learning/`, `cognitive-science/`, `statistics/`)* | *(e.g., `Deep_Learning_Architectures.md`, `Neural_Network_Theory.md`, `Statistical_Inference.md`)* |
| **`business/`** | Organizational behavior, management strategy, enterprise AI, market dossiers. | *(e.g., `strategy/`, `market-research/`, `management/`)* | *(e.g., `Enterprise_AI_Strategy.md`, `Organizational_Behavior.md`, `Market_Analysis.md`)* |
| **`career/`** | Master resumes/CVs, target companies, interview prep, skill matrices. | *(Top-level flat)* | *(e.g., `Master_Resume_Dossier.md`, `Target_Companies_Dossier.md`, `Technical_Interview_Preparation.md`)* |
| **`dev/`** | Software engineering, data pipelines, web scrapers, DevOps & MLOps. | *(e.g., `pipelines/`, `algorithms/`, `web-scraping/`)* | *(e.g., `Data_Pipeline_Architecture.md`, `Web_Scraper_Design.md`, `API_Design_Patterns.md`)* |
| **`people/`** | Academic collaborators, mentors, authors, industry leaders (`type: person`). | *(Top-level flat)* | *(e.g., `Alan_Turing.md`, `Claude_Shannon.md`, `Collaborator_Profiles.md`)* |
| **`personal/`** | Personal reflections, learning milestones, productivity workflows, administration. | *(Top-level flat)* | *(e.g., `Yearly_Goals_And_Reflections.md`, `Certification_Study_Guide.md`, `Reading_Log.md`)* |
| **`projects/`** | Active engineering and academic research initiatives (`type: project`). | *(e.g., `coursework/`, `open-source/`, `research-initiatives/`)* | *(e.g., `Knowledge_Graph_Pipeline.md`, `Open_Source_Contribution.md`)* |
| **`tools/`** | Software platforms, hardware instruments, developer utilities (`type: tool`). | *(Top-level flat)* | *(e.g., `Obsidian_Second_Brain_Architecture.md`, `Browser_Automation_Tools.md`)* |
...
3. **Permanent Archive (`raw/processed/`)**:
   - Upon successful ingestion and reduction by `scripts/reduce.py`, raw source files are cleanly archived into `raw/processed/` to guarantee idempotency and prevent duplicate processing.
...
- **Newsletter Blacklist**: Automatically detects and tags automated system emails (`no-reply@*`, notifications, marketing newsletters) as `tags: [email-newsletter]` to prevent spam from contaminating the graph.
- **Output**: Writes `raw/imports/outlook_emails.json`, converted to markdown in `raw/assets/emails/` via `scripts/extract_emails.py`.
...
- **Output**: Generates `reports/lint_report.md` with overall health status (`🟢 Green`, `🟡 Yellow`, or `🔴 Red`).
...
| **2** | **Type Validation** | **Structural** | Note `type` is not one of the valid types or misses type-specific required fields. |
